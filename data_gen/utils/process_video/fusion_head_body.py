# gen_head.mp4 is a 11s video, first cut raw.mp4 to 11s
# ffmpeg -ss 00:00:35 -i raw.mp4 -t 00:00:11 -c copy body.mp4
# body.mp4 1080*1920
# gen_head.mp4 512*512
# 1 Convert body.mp4 to image bodies
# 2 Convert gen_head.mp4 to image heads
# 3 Separate the characters and backgrounds in bodies
# 4 Overlay multiple backgrounds (assuming the background is still). This operation is optional.
# 5 Identify the head of bodies and remove it
# 6 Separate the head of heads
# 7 Splice heads and bodies
""" You can use the following method to extract the image, but you need to avoid resizing it to 512*512
export PYTHONPATH=./
...
"""
# The directory for data is data_fusion, and the subdirectories are audio, head, body and output
# The audio needs to be sampled at 16K, the head completes the generation operation, and the body is the original cut-out image
# Extract the image with the neck for easy splicing
# Splice the torso_imgs of data_fusion/body and the head_neck_imgs of data_fusion/head
...
def load_segment_mask_from_file(filename: str):
    ...
    return decode_segmap_mask_from_image(encoded_segmap)
...
def generate_segment_imgs_job(img_name, segmap, img):
    ...
    out_img_name = img_name.replace(
        "/gt_imgs/", "/inpaint_torso_imgs/"
    ).replace(".jpg", ".png")
...
def segment_and_generate_for_image_job(img_name, img, segmenter_options=None, segmenter=None, store_in_memory=False):
    ...
    return segmap_mask if store_in_memory else segmap_name
...
def extract_segment_job(
    ...
    try:
        ...
        for img_name in img_names:
            img = load_rgb_image_to_path(img_name) if store_in_memory else img_name
            img_lst.append(img)
        ...
        if multiprocess_enable:
            segmap_mask_lst.extend(
                res
                for _, res in multiprocess_run_tqdm(
                    segment_and_generate_for_image_job,
                    args=args,
                    num_workers=16,
...
def overlay_images(body_img_path, head_img_path, output_img_path, position):
    """
    Paste the head image to the specified position on the body image and save the composite image.
    
    :param body_img_path: path to the body image
    :param head_img_path: path to the head image
    :param output_img_path: the path of the output composite image
    :param position: tuple (x, y) of the paste position
    """
    body_img = Image.open(body_img_path)
    head_img = Image.open(head_img_path)

    body_img.paste(head_img, position, head_img)

    body_img.save(output_img_path)

def overlay_images_cv2(body_img_path, head_img_path, output_img_path, position):
    """
    Paste the head image with alpha channel onto the body image at the specified position and save the composite image.
    :param body_img_path: path to the body image
    :param head_img_path: path to the head image
    :param output_img_path: The path of the output composite image
    :param position: tuple (x, y) of the paste position
    """
    # Read body image and head image
    body_img = cv2.imread(body_img_path, cv2.IMREAD_UNCHANGED)
    head_img = cv2.imread(head_img_path, cv2.IMREAD_UNCHANGED)

    # Make sure the header image has an alpha channel
    ...
    # Make sure the paste position is within the bounds of the target image.
    x, y = position
    if x + head_w > body_w or y + head_h > body_h:
        raise ValueError("The head image extends beyond the body image")

    # Create a copy of the composite image
...
def composite_images(body_img_path, head_img_path, output_img_path, position):
    ...
    result_img.paste(overlay_img, position, overlay_img)  # overlay_img
...
def fusion_head_body():
    ...
    min_len = min(body_imgs_len, head_imgs_len)
    ...
    for i in range(min_len):
        ...
        print(f"{head_img_name}:{body_img_name}")
